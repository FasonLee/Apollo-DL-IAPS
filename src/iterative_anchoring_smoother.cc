...
#include "proto/planner_open_space_config.pb.h"
...
IterativeAnchoringSmoother::IterativeAnchoringSmoother(
  ...
  ego_length_ = 5.033;
  ego_width_ = 1.989;
  back_edge_to_center_ = 0.940;
  center_shift_distance_ =  ego_length_ / 2.0 - back_edge_to_center_;
...
bool IterativeAnchoringSmoother::Smooth(
  ...
  if (xWS.cols() < 2) {
    std::cout << "reference points size smaller than two, smoother early "
              "returned" << std::endl;
    return false;
  }
  ...
  if (interpolated_size < 4) {
    std::cout << "interpolated_warm_start_path smaller than 4, can't enforce "
              "heading continuity" << std::endl;
    return false;
  } else if (interpolated_size < 6) {
    std::cout << "interpolated_warm_start_path smaller than 4, can't enforce "
              "initial zero kappa" << std::endl;
  ...
  if (!GenerateInitialBounds(interpolated_warm_start_path, &bounds)) {
    std::cout << "Generate initial bounds failed, path point to close to obstacle" << std::endl;
    return false;
  }
  ...
  if (!CheckCollisionAvoidance(interpolated_warm_start_path,
    ...
    std::cout << "Interpolated input path points colliding with obstacle" << std::endl;
    
    if (!ReAnchoring(input_colliding_point_index_, &interpolated_warm_start_path)) {
      std::cout << "Fail to reanchor colliding input path points" << std::endl;
      return false;
    }
  }
  ...
  std::cout << "iterative anchoring path smoother time: "
         << path_smooth_diff.count() * 1000.0 << " ms." << std::endl;
  ...
  if (!SmoothSpeed(init_a, init_v, smoothed_path_points_.Length(),
                   &smoothed_speeds)) {
    return false;
  }
  ...
  //         init_a, init_v, smoothed_path_points_.Length(), &smoothed_speeds)) {
  //   return false;
  // }
  ...
  std::cout << "iterative anchoring speed smoother time: "
         << speed_smooth_diff.count() * 1000.0 << " ms." << std::endl;

  // Combine path and speed
  if (!CombinePathAndSpeed(smoothed_path_points_, smoothed_speeds,
  ...
  std::cout << "iterative anchoring smoother total time: " << diff.count() * 1000.0
         << " ms." << std::endl;

  std::cout << "discretized_trajectory size " << discretized_trajectory->size() << std::endl;
  return true;
}
...
bool IterativeAnchoringSmoother::ReAnchoring(
  ...
  if (colliding_point_index.empty()) {
    std::cout << "no point needs to be re-anchored" << std::endl;
    return true;
  }

  for (const auto index : colliding_point_index) {
    if (index == 0 || index == path_points->size() - 1) {
      std::cout << "Initial and end points collision avoid condition failed." << std::endl;
      return false;
    }
    if (index == 1 || index == path_points->size() - 2) {
      std::cout << "second to last point or second point pos reanchored. Heading "
                "discontinuity might "
                "happen" << std::endl;
    }
  }
  ...
  for (const auto index : colliding_point_index) {
    ...
    if (!reanchoring_success) {
      std::cout << "interpolated points at index " << index
             << "can't be successfully reanchored" << std::endl;
      return false;
    }
  }
  return true;
...
bool IterativeAnchoringSmoother::SmoothPath(
  ...
  while (!is_collision_free) {
    if (counter > max_iteration_num) {
      std::cout << "Maximum iteration reached, path smoother early stops" << std::endl;
      return true;
    }
    ...
    if (opt_x.size() < 2 || opt_y.size() < 2) {
      std::cout << "Return by fem_pos_smoother is wrong. Size smaller than 2 " << std::endl;
      return false;
    }
      ...
      for (const LineSegment2d& linesegment : obstacle_linesegments) {
        if (ego_box.HasOverlap(linesegment)) {
          ...
          std::cout << "point at " << i << "collied with LineSegment "
                 << linesegment.DebugString() << std::endl;

          collided_ego_box_.emplace_back(ego_box.get_corners());
...
bool IterativeAnchoringSmoother::SmoothSpeed(const double init_a,
  ...
  std::cout << "total_t is : " << total_t << std::endl;
...
bool IterativeAnchoringSmoother::CombinePathAndSpeed(
  ...
  if (path_points.empty()) {
    std::cout << "path data is empty" << std::endl;
    return false;
  }
  std::cout << "speed_points.TotalTime() " << speed_points.TotalTime() << std::endl;
  for (double cur_rel_time = 0.0; cur_rel_time < time_horizon;
    ...
    if (!speed_points.EvaluateByTime(cur_rel_time, &speed_point)) {
      std::cout << "Fail to get speed point with relative time " << cur_rel_time << std::endl;
      return false;
    }
  ...
  std::cout << "path length before combine " << path_points.Length() << std::endl;
  std::cout << "trajectory length after combine "
         << discretized_trajectory->GetSpatialLength() << std::endl;
  return true;
}
...
bool IterativeAnchoringSmoother::GenerateStopProfileFromPolynomial(
  ...
  std::cout << "GenerateStopProfileFromPolynomial fails." << std::endl;
  return false;
}
