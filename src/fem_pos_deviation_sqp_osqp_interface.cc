...
#include "include/fem_pos_deviation_sqp_osqp_interface.h"
...
bool FemPosDeviationSqpOsqpInterface::Solve() {
  ...
  if (ref_points_.empty()) {
    std::cout << "reference points empty, solver early terminates" << std::endl;
    return false;
  }

  if (ref_points_.size() != bounds_around_refs_.size()) {
    std::cout << "ref_points and bounds size not equal, solver early terminates" << std::endl;
    return false;
  }

  if (ref_points_.size() < 3) {
    std::cout << "ref_points size smaller than 3, solver early terminates" << std::endl;
    return false;
  }

  if (ref_points_.size() > std::numeric_limits<int>::max()) {
    std::cout << "ref_points size too large, solver early terminates" << std::endl;
    return false;
  }
  ...
  while (pen_itr < sqp_pen_max_iter_) {
    ...
    while (sub_itr < sqp_sub_max_iter_) {
      ...
      if (!iterative_solve_res) {
        std::cout << "iteration at " << sub_itr
               << ", solving fails with max sub iter " << sqp_sub_max_iter_ << std::endl;
      ...
      if (ftol < sqp_ftol_) {
        std::cout << "merit function value converges at sub itr num " << sub_itr << std::endl;
        std::cout << "merit function value converges to " << cur_fvalue
               << ", with ftol " << ftol << ", under max_ftol " << sqp_ftol_ << std::endl;
        fconverged = true;
        break;
      }
    ...
    if (ctol < sqp_ctol_) {
      std::cout << "constraint satisfied at pen itr num " << pen_itr << std::endl;
      std::cout << "constraint voilation value drops to " << ctol
             << ", under max_ctol " << sqp_ctol_ << std::endl;
      weight_curvature_constraint_slack_var_ = original_slack_penalty;
      osqp_cleanup(work);
      c_free(data->A);
      c_free(data->P);
  ...
  std::cout << "constraint not satisfied with total itr num " << pen_itr << std::endl;
  std::cout << "constraint voilation value drops to " << ctol
         << ", higher than max_ctol " << sqp_ctol_ << std::endl;
  weight_curvature_constraint_slack_var_ = original_slack_penalty;
  osqp_cleanup(work);
  c_free(data->A);
  c_free(data->P);
